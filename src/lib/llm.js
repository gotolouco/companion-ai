 ...
 * @param {Array<{sender: 'user'|'bot', text: string}>} history - histórico da conversa
 * @param {string} [personalidade] - personalidade escolhida no painel
  ...
  try {
    const response = await fetch(API_URL, {
      ...
      body: JSON.stringify({
        model: MODEL,
        messages,
        temperature: 0.9,   // respostas mais naturais e variadas
        max_tokens: 300,    // limita o tamanho, evitando "textão"
      }),
    })
